...
def generate_strong_weak(mdp_name, feat_to_incl):
    ...
    return MDP, strong_student, weak_student, policy
...
def run_plot(original, strong, weak, policy, n_sims, sim_steps, ax):
    np.random.seed(100)
    ...
    rewards_record = rewards_record.melt(value_vars=['original', 'strong', 'weak'], id_vars = ['step'], var_name = 'student_type', 
...
for ax, (mdp_name, feat_to_incl) in enumerate(mdp_dict.items()):
    ...
    original, strong, weak, policy = generate_strong_weak(mdp_name = mdp_name, feat_to_incl=feat_to_incl)

    run_plot(original,strong,weak,policy,n_sims,sim_length,axs[ax])
...
original, strong, weak, policy = generate_strong_weak(mdp_name = mdp_name, feat_to_incl=feat_to_incl)
run_plot(original,strong,weak,policy,n_sims,sim_length,axs[ax + 1])
