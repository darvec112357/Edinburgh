...
void execute()
{
    ...
    switch (control->PCSource) {
        case 0:
            ...
            break;
        case 1://BEQ
            if(alu_opA==alu_opB) next_pipe_regs->pc = next_pipe_regs->ALUOut;
        case 2://JUMP
            next_pipe_regs->pc =get_piece_of_a_word(curr_pipe_regs.pc-4,28,32)<<28+(IR_meta->jmp_offset)<<2
        default:
            assert(false);
    }
}
...
void memory_access() {
    ...
    struct pipe_regs *next_pipe_regs = &arch_state.next_pipe_regs;
     ...
     if(control->MemRead&&control->IorD){
       next_pipe_regs->MDR=memory_read(alu_opB);
     }
     if(control->MemWrite&&control->IorD){
       memory_write(alu_opB,next_pipe_regs->ALUOut);
     }
}
...
void write_back()
{
    ...
    if(!arch_state.control.RegDst&&arch_state.control.RegWrite&&arch_state.control.MemtoReg){
