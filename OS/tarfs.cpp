...
int TarFSFile::pread(void* buffer, size_t size, off_t off)
{
	...
	unsigned int filesize=this->size();
	...
	unsigned int block_idx=off/512;
	off_t remainder=off%512;
	unsigned int offset=size+remainder;

	if(offset<=filesize){
		num_blocks=offset/512;
		if(offset%512>0){
			num_blocks++;
		}
	}
		...
		answer=filesize-remainder;
	}

	char* b=new char[512*num_blocks];
	_owner.block_device().read_blocks(b, block_idx + _file_start_block, num_blocks);
	const char *original = b + remainder;
	delete b;
...
TarFSNode* TarFS::build_tree()
{
	...
	TarFSNode *answer= new TarFSNode(NULL, "", *this);
	TarFSNode *copy=answer;

	// You must read the TAR file, and build a tree of TarFSNodes that represents each file present in the archive.
	unsigned int filesize;
	infos::util::String path;
	infos::util::String filename;
	size_t total = block_device().block_count();

	for (unsigned int idx = 0; idx < total; idx+=filesize + 1) {
		struct posix_header *header = (struct posix_header *) new char[block_device().block_size()];
		block_device().read_blocks(header,idx,1);

		if (is_zero_block((uint8_t *) header) && idx<block_device().block_count()-1) {
			uint8_t *zero = new uint8_t[block_device().block_size()];
			block_device().read_blocks(zero, idx + 1, 1);
			if (is_zero_block(zero)) {
				delete zero;
				delete header;
				break;
			}
		...
		path=String(header->name);
		List<String> paths=path.split('/',true);
		if(paths.empty()){
			filename="/";
		}
		else{
		...
		if (paths.count() == 1) {
			...
			String directory = paths.at(paths.count() - 2);
      String lead = (String) copy->name();
			unsigned int i=0;
			while(infos::util::strncmp(lead.c_str(),directory.c_str(),lead.length()) &&
		        (lead.length()!=directory.length()) && i<paths.count()){
				copy = (TarFSNode *) copy->parent();
        lead = (String) copy->name();
				i++;
			}
		}

		filesize = octal2ui(header->size)/512;

		if((octal2ui(header->size))%512>0){
			filesize++;
		}

		TarFSNode *current = new TarFSNode(copy, filename, *this);

		current->set_block_offset(idx);
		current->size(octal2ui(header->size));
		copy->add_child(filename,current);
